...
public class SimpleHillClimbing {
    ...
    static int arr[][] = new int[4][2];//to store potential moves 
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (current[i][j] == 0) {
                        x = i;
                        y = j;
                    }//x,y stores the value of cord where the '0' is present!
                }
            }
            mov = findIndex(x, y, arr); //findngings no. of moves possible!
            float bestHval = ans;
